...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    var scalarMult = x1 * x2 + y1 * y2;
    var lenthMult = Math.sqrt(x1 * x1 + y1 * y1) * Math.sqrt(x2 * x2 + y2 * y2);
    return Math.acos(scalarMult / lenthMult);
}
...
function roundToPowerOfTen(num, pow) {
    ...
    var remainder = num % tenPow;
    if (pow === 0) {
        return num;
    } else {
        if (remainder < tenPow / 2) {
            return num - remainder;
        } else {
            return num - remainder + tenPow;
        }
    }
}
