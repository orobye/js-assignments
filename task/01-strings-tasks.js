...
function removeFirstOccurrences(str, value) {
    ...
    for (var i = 0; i < str.length; i++) {
        if (str.slice(i, i + valueLength) === value) {
            start = str.slice(0, i);
            end = str.slice(i + valueLength, str.length);
            str = `${start}${end}`;
            break;
        }
    }
...
function getRectangleString(width, height) {
    ...
    for (var h = 0; h < height; h++) {
        ...
        if (h === 0) { object+= '┌'; }
        else if (h > 0 && h < height - 1) { object += '│'; }
        else if (h === height - 1) { object += '└'; }

        for (var w = 0; w < width - 2; w++) {
            if (h === 0 || h === height - 1) { object += '─'; }
            else { object += ' '; }
        }
...
function encodeToRot13(str) {
    ...
    var nop = "NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm";
    var result = "";
    
    for (var i = 0; i < str.length; i++) {
        if (abc.includes(str[i])) {
            var position = abc.indexOf(str[i]);
            result += nop[position];
        } else {
            result += str[i];
        }
